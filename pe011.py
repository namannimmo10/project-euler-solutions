...
grid_vals = [i.split() for i in grid]
grid_vals = [[int(j) for j in i] for i in grid_vals]
...
for i in range(20):
    for j in range(16):
        # up and down
        prod = grid_vals[j][i] * grid_vals[j+1][i] * grid_vals[j+2][i] *  grid_vals[j+3][i]
        if prod > prod_max:
            prod_max = prod
        
        # left and right
        prod = grid_vals[i][j] * grid_vals[i][j+1] * grid_vals[i][j+2] * grid_vals[i][j+3]
        if prod > prod_max:
            prod_max = prod
...
for i in range(16):
    for j in range(16):
        prod = grid_vals[i][j] * grid_vals[i+1][j+1] * grid_vals[i+2][j+2] * grid_vals[i+3][j+3]
        if prod > prod_max:
            prod_max = prod
